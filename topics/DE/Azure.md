...
Azure Data Factory(ADF) — облачная служба ETL, ELT, которая используется для перемещения и трансформации данных, которые
могут иметь разные
...
DataSet - представление данных, которое указывает или ссылается на данные, которые можно использовать в качестве входных
и выходных данных.
...
1. Lift and shift
   включают как «классический» сценарий подъема и сдвига, когда приложение и его данные перемещаются в Azure, так и
   «гибридный» сценарий подъема и сдвига, когда данные приложения перемещаются в файлы Azure, а приложение продолжает
   выполняться локально.

- Storage Queue - получение доступа к данным через утентифицированные вызовы с использованием HTTP или HTTPS. Размер
  сообщения очереди может достигать 64 КБ.
...
Вы можете использовать конфигурацию приложений Azure для хранения и извлечения параметров приложений, микросервисов,
платформ и конвейеров CI/CD.
